...
type Member = (typeof data)[number];

const DataTable = () => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [checkedKeys, setCheckedKeys] = useState<number[]>([]);
  const [sortColumn, setSortColumn] = useState<keyof Member | undefined>();
  ...
  const handleCheckAll = (_value: unknown, checked: boolean) => {
  ...
  const handleSortColumn = (dataKey: string, sortType?: 'asc' | 'desc') => {
    setSortColumn(dataKey as keyof Member);
  ...
  return (
    <>
      ...
      <Table
        height={typeof window !== 'undefined' ? Math.max(getHeight(window) - 200, 400) : 600}
        data={filteredData()}
